...
// Import statically for now but structured as if it were an API call
import venuesRaw from '../data/venues.json';

const venuesData = venuesRaw as any[];
...
export default function SearchBox() {
  ...
  useEffect(() => {
    ...
    if (city) filtered = filtered.filter((v: any) => v.city === city);
    if (country) filtered = filtered.filter((v: any) => v.country === country);
    setResults(filtered);
    setShowCount(18);
  }, [query, city, country]);
        ...
        {results.slice(0, showCount).map((venue: any) => (
          <a key={venue.slug} href={`/venue/${venue.slug}`}
            ...
            <div className="h-44 overflow-hidden relative">
              <img src={venue.image?.[0] || 'https://images.unsplash.com/photo-1514525253361-bee8a48790c3?w=600'} 
                alt={venue.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
              {venue.city && (
            ...
            <div className="p-5">
              <div className="flex items-start gap-3 mb-2">
                {venue.logo?.[0] && <img src={venue.logo[0]} alt="" className="w-8 h-8 rounded-full object-cover flex-shrink-0 mt-0.5 border border-gray-100" />}
